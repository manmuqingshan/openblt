...
void UsbInit(void)
{
  ...
  /* after a reset, the USB_OTG_GCCFG_NOVBUSSENS bit is 0, meaning that the USB core
   * expects the Vbus pin (PA9 typically) to be configured and used for sensing the Vbus
   * voltage level.
   * explicitly disable the Vbus sensing feature to (a) free up the use of this pin, and
   * (b) make sure the USB device can actually enumerate if the Vbus pin is not actually
   * properly configured and connected to Vbus. 
   * after disabing Vbus sensing, the USB core simply assumes that Vbus is always at
   * a valid level, being greater than 4.25V.
   */
