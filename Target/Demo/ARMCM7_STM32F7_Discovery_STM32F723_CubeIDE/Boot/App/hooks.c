/************************************************************************************//**
* \file         Demo/ARMCM7_STM32F7_Discovery_STM32F723_CubeIDE/Boot/App/hooks.c
* \brief        Bootloader callback source file.
* \ingroup      Boot_ARMCM7_STM32F7_Discovery_STM32F723_CubeIDE
* \internal
*----------------------------------------------------------------------------------------
*                          C O P Y R I G H T
*----------------------------------------------------------------------------------------
...
void EventsHook(tEventsId id, void const *info)
{
  ...
  switch (id)
  {
    case EVENT_ID_ON_START:
      ...
      /* For firmware updates from a local file system, the filename is not NULL. */
    ...
    case EVENT_ID_ON_ERASE:
      /* Indicate some progress and update the displayed text. */
      DisplaySetProgress(2);
      DisplaySetText("Erasing..");
      break;

    case EVENT_ID_ON_WRITE:
      progress  = ((tEventsInfoWrite const *)info)->progress;
      /* Update progress. Note that first two percent was used to indicate erase. */
      if (progress > 2U)
      {
        DisplaySetProgress(progress);
      }
      /* Update the displayed text. */
      DisplaySetText("Programming..");
      break;

    case EVENT_ID_ON_SUCCESS:
      /* Firmware update from local file system completed? */
    ...
    case EVENT_ID_ON_ERROR:
      ...
      /* Show error code on the display. */
      sprintf(errStr, "Error #%02u", error_id);
      DisplaySetText(errStr);
      break;
