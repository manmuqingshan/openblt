...
void HAL_MspDeInit(void)
{
  ...
  /* Reset GPIO pin for turning off the power to the USB HS Host. */
  LL_GPIO_ResetOutputPin(GPIOH, LL_GPIO_PIN_12);

  /* Disable the USB HS clock and Deinit the related GPIO pins. */
  HCD_HandleTypeDef usbHandle;
  usbHandle.Instance = USB_OTG_HS;
  HAL_HCD_MspDeInit(&usbHandle);
