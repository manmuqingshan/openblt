/************************************************************************************//**
* \file         Demo/ARMCM0_STM32C0_Nucleo_C071RB_CubeIDE/Boot/App/hooks.c
...
#include "usbcomposite.h"                        /* USB composite device for OpenBLT   */
...
blt_bool BackDoorEntryHook(void)
{
  ...
  /* to activate the bootloader, keep the pushbutton pressed during a reset. */
  if (LL_GPIO_IsInputPinSet(GPIOC, LL_GPIO_PIN_13) == 0)
  {
    /* pushbutton pressed, which means a backdoor entry is requested. */
    result = BLT_TRUE;
  }
...
blt_bool CpuUserProgramStartHook(void)
{
  ...
  /* release the USB composite device. */
  UsbCompositeFree();
